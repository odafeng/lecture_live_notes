...
- 筆記面板下方的輸入框：`✍️ 筆記` 記下你自己的想法，`⟲ 更正` 指正 AI 寫錯的地方
- `自動捲動`：可關閉以閱讀前面的內容，開啟後回到最新記錄
- 下課後：文件排版的完整筆記，所有輸出檔案的下載按鈕，以及整包 ZIP
...
# 12b. 自己寫筆記，以及更正寫錯的 AI

筆記面板下方有一個輸入框，兩種模式，按 Enter 送出。

## `✍️ 筆記`

記下你自己的想法。內容會立刻寫進 `live_notes.md`：

```text
> ✍️ [00:12:35] 老師說這頁會考
```

下課整併時，它有兩個去處：作為**最高優先的素材**交給 Claude 整進正文，同時由後端在
`final_notes.md` 末尾原文附上一整段 `## 我的課堂筆記（原文）`。原文那一段是程式拼接的，
不是請模型照抄，所以你寫的字一個都不會被改寫掉。

## `⟲ 更正`

指正 AI 寫錯的地方，或校正整理的方向：

```text
> ⟲ 更正 [00:20:01] 老師說的是 ResNet，不是 resonate
```

更正是累積式的**指示**，不是素材。送出後會注入之後每一次生成：逐段筆記、章節整併、
最終整併，全部都帶著它跑，並明確要求「與更正衝突時以更正為準」。

所以已經寫錯的那一段不會當場重寫 —— 它會在下課整併時被覆蓋掉。這是刻意的取捨：
重跑單一段落要多付一次 API 費用、要管重跑中的狀態，而你真正要複習的是最終筆記。
`live_notes.md` 裡錯的那段保留原樣，下方留著你的更正，事後看得出來發生過什麼。

更正不會出現在 `## 我的課堂筆記（原文）` —— 它是給模型的指示，不是複習內容。

兩者都存進 `finalize_input.json`，所以按「重新整併」重跑時一樣生效。
...
這一段由後端原文拼接，不經過模型，所以一字不漏。

---

## `final_notes.html`

跟 `final_notes.md` 同一份內容，轉成一個獨立的 HTML 檔。樣式內嵌、不連外部資源、不含
script，所以離線打得開，也可以直接列印或寄給同學。

Markdown 適合再加工，HTML 適合直接讀。兩個都留。

---

## 整包 ZIP

下載區的「整包下載」會把這堂課的所有檔案壓成一個 ZIP：錄音、逐字稿、即時筆記、
完整筆記（Markdown 與 HTML）、課堂資訊。檔名用課程名稱加上課時間，例如
`機器學習_20260907_090001.zip`。

ZIP 是即時壓縮出來的，不會在 `lectures/` 留下額外檔案。`finalize_input.json` 不放進去
—— 那是重新整併用的內部素材，不是你要帶走的東西。

---

## `finalize_input.json`

整併完整筆記時用的素材，包含課程名稱、章節摘要、還沒併進章節的最後幾段筆記，以及你在
課堂上輸入的手寫筆記與更正。
